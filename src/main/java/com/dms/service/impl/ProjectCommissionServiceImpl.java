...
public class ProjectCommissionServiceImpl implements ProjectCommissionService {
    ...
    private void updateCommission(Long employeeId, BigDecimal commission, LocalDateTime commissionDate) {
        ...
        if (existFinanceDtos != null && existFinanceDtos.size() > 0) {
            ...
            financeDto.setCommission(commission);
    ...
    @Override
    public void revertFirstCommissions(List<ProjectCommissionDto> projectCommissionDtos) {
        projectCommissionDao.revertFirstCommissions(projectCommissionDtos);
    }

    @Override
    public void revertBalanceCommissions(List<ProjectCommissionDto> projectCommissionDtos) {
        projectCommissionDao.revertBalanceCommissions(projectCommissionDtos);
    }
    ...
    @Override
    public List<ProjectCommissionDto> getCurrentMonthProjectCommissions(String currentMonth) {
        return projectCommissionDao.getCurrentMonthProjectCommissions(currentMonth);
    }
    ...
    public void updateDesignAssistants(List<DesignAssistantDto> designAssistantDtos) {
        for (DesignAssistantDto designAssistantDto : designAssistantDtos) {
            ProjectCommissionDto extProjectCommissionDto = null;
            ProjectCommissionFilterRequest projectCommissionFilterRequest = new ProjectCommissionFilterRequest();
            projectCommissionFilterRequest.setAcNumber(designAssistantDto.getAcNumber());
            List<ProjectCommissionDto> projectCommissionDtoList = projectCommissionDao.getProjectCommissions(projectCommissionFilterRequest);
            if (projectCommissionDtoList != null) {
                extProjectCommissionDto = projectCommissionDtoList.get(0);
            }
            if (extProjectCommissionDto != null) {
                ...
                if (employee != null && !StringUtils.isEmpty(employee.getId())) {
                    ...
                    if (!StringUtils.isEmpty(extProjectCommissionDto.getBranch()) && (DmsConstants.SUBURBS_JIADING.equals(extProjectCommissionDto.getBranch())
                            || DmsConstants.SUBURBS_SONGJIANG_DIAN.equals(extProjectCommissionDto.getBranch()))) {
                        designAssistantDto.setDesignerAssistantCommissionRate(DmsConstants.DESIGN_ASSISTANT_SUBURBS_COMMISSION_RATE.setScale(4, BigDecimal.ROUND_HALF_UP));
                        LOGGER.error(extProjectCommissionDto.getBranch() + DmsConstants.DESIGN_ASSISTANT_SUBURBS_COMMISSION_RATE);
                    } else {
                        designAssistantDto.setDesignerAssistantCommissionRate(DmsConstants.DESIGN_ASSISTANT_COMMISSION_RATE.setScale(4, BigDecimal.ROUND_HALF_UP));
                    }

                    designAssistantDto.setDesignerAssistantCommission(extProjectCommissionDto.getCommissionBase()
                            .multiply(designAssistantDto.getDesignerAssistantCommissionRate())
                            .setScale(2, BigDecimal.ROUND_HALF_UP));

                    designAssistantDto.setDesignerAssistantCommissionDate(new LocalDateTime(DateUtils.getPreviousMonth()));
                    updateCommission(designAssistantDto.getDesignAssistantId(), designAssistantDto.getDesignerAssistantCommission(),
                            designAssistantDto.getDesignerAssistantCommissionDate());
